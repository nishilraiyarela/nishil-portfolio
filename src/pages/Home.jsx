...
import { Github, Linkedin, Instagram, Twitter, ArrowUpRight, Download, X } from "lucide-react";
    ...
    >
      <div className="flex items-center gap-6 pointer-events-auto">
        <div
          ...
          className="text-2xl font-black tracking-tighter hover:opacity-70 transition-opacity cursor-pointer text-white"
        >
          NR
        </div>
      ...
      <div className="pointer-events-auto flex items-center gap-4 md:gap-6">
        ...
        >
          <span className="text-[9px] uppercase tracking-[0.25em] font-bold text-zinc-400 group-hover:text-white leading-none">
            CV
          </span>
        ...
        <button onClick={() => scrollToSection("contact")} className="group relative px-2 py-1 overflow-hidden">
          <span className="text-[11px] uppercase tracking-[0.3em] font-black text-zinc-400 group-hover:text-white transition-colors duration-300">
            Get in touch
          </span>
...
export default function Home() {
  ...
  useEffect(() => {
    const calc = () => {
      ...
      const maxDrag = Math.max(0, scrollWidth - containerWidth);
    ...
    <main className="relative w-full bg-zinc-950 text-zinc-100 overflow-x-hidden selection:bg-white selection:text-black">
      ...
      <section id="home" className="relative h-screen flex items-center justify-center">
        <div className="relative z-10 flex flex-col items-center translate-y-24 text-center">
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-7xl md:text-9xl font-black tracking-tighter text-white uppercase leading-none">
            NISHIL
          </motion.h1>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 0.4 }} className="mt-8 text-zinc-500 text-[10px] md:text-[12px] font-medium tracking-[0.8em] uppercase">
      ...
      <section id="work" className="relative z-30 bg-zinc-950 py-24 border-t border-zinc-900/50">
        <div className="max-w-7xl mx-auto px-6 md:px-20 mb-14 text-center">
          <span className="text-[10px] uppercase tracking-[0.5em] font-bold text-zinc-500 opacity-40 mb-8 block">02 / Portfolio</span>
          <h2 className="text-4xl md:text-7xl font-light tracking-tighter text-zinc-400">
            Selected <span className="italic text-white font-serif">works</span>.
          </h2>
        </div>
        ...
        <div ref={containerRef} className="overflow-x-hidden relative">
          <motion.div
            ...
            className="flex gap-10 px-6 md:px-20 pb-8 cursor-grab active:cursor-grabbing"
          >
            {PROJECTS_DATA.map((project) => (
              <motion.div
                key={project.id}
                onClick={() => !isDragging && setSelectedProject(project)}
                className="min-w-[350px] md:min-w-[600px] group select-none"
              >
                <div className="relative aspect-[16/10] overflow-hidden bg-zinc-900 rounded-sm">
                  <img
                ...
                <div className="mt-6 flex justify-between items-end border-b border-zinc-900 pb-6">
                  <div>
                    <h3 className="text-2xl font-bold text-zinc-300 uppercase tracking-tighter">{project.title}</h3>
                    <p className="text-zinc-600 text-[10px] mt-2 uppercase tracking-widest font-bold">{project.category}</p>
                  </div>
                  <div className="h-10 w-10 rounded-full border border-zinc-900 flex items-center justify-center">
  ...
  {selectedProject && (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-6xl bg-zinc-950 border border-zinc-800 rounded-sm overflow-hidden flex flex-col"
      ...
      >
        ...
        <div className="flex flex-col md:flex-row border-b border-zinc-900">
          ...
          <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-start bg-zinc-950">
            <span className="text-[10px] uppercase tracking-[0.5em] font-bold text-emerald-500 mb-6 block">Project Overview</span>
          ...
          {/* RIGHT: Dynamic Media Handler (Unified 16:9 Ratio) */}
          <div className="w-full md:w-1/2 aspect-video bg-zinc-900 border-l border-zinc-900 relative flex items-center justify-center overflow-hidden">
            {(() => {
            ...
            {/* Close Button stays on top of media */}
            <button
              onClick={() => setSelectedProject(null)}
              className="absolute top-6 right-6 z-50 p-2 rounded-full bg-black/50 border border-white/10 text-white hover:bg-white hover:text-black transition-all"
            >
              <X size={20} />
            </button>
          </div>
        ...
        {/* BOTTOM SECTION: Title & Stacked Links */}
        <div className="p-8 md:p-12 bg-zinc-900/20 flex flex-col gap-8">
          <div className="space-y-2">
            <p className="text-[10px] uppercase tracking-[0.4em] font-bold text-zinc-500">
              {selectedProject.category}
            </p>
            <h3 className="text-3xl md:text-5xl font-black tracking-tighter text-white uppercase leading-none">
              {selectedProject.title}
            </h3>
          </div>

          <div className="flex flex-col gap-3 max-w-fit">
            {selectedProject.github && selectedProject.github !== "null" ? (
              <a
                href={selectedProject.github}
                target="_blank"
                rel="noopener noreferrer"
                className="group flex items-center gap-3 px-8 py-3 rounded-full border border-zinc-800 bg-zinc-950 text-white hover:border-zinc-400 transition-all"
              >
                <Github size={16} />
                <span className="text-[10px] uppercase tracking-widest font-black">Repository</span>
              </a>
            ) : (
              <div className="flex items-center gap-3 px-8 py-3 rounded-full border border-zinc-900 bg-zinc-900/30 text-zinc-600 cursor-default">
                <X size={14} />
                <span className="text-[10px] uppercase tracking-widest font-black">Source Private</span>
              </div>
            )}
            ...
            {selectedProject.live && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="group flex items-center gap-3 px-8 py-3 rounded-full bg-emerald-500 text-black hover:bg-emerald-400 transition-all"
              >
                <ArrowUpRight size={16} />
                <span className="text-[10px] uppercase tracking-widest font-black">Live Project</span>
              </a>
            )}
          </div>
        </div>
      ...
      {/* 03. SKILLS SECTION */}
      <section id="skills" className="relative z-30 bg-zinc-950 py-24 border-t border-zinc-900/50 overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 md:px-20">
          <div className="flex flex-col items-center mb-16">
            <span className="text-[10px] uppercase tracking-[0.5em] font-bold text-zinc-500 opacity-40 mb-8 block">03 / Tech Stack</span>
            <h2 className="text-4xl md:text-7xl font-light tracking-tighter text-zinc-400 text-center leading-[1.1]">
                  ...
                  <div className="flex flex-wrap gap-x-8 gap-y-4">
                    {skill.items.map((item, idx) => (
                      <div key={idx} className="flex items-center gap-3">
                        <div className="w-1.5 h-1.5 bg-zinc-800 group-hover:bg-emerald-500 transition-all duration-500" />
                        <span className="text-lg md:text-xl font-light text-zinc-500 group-hover:text-white transition-colors duration-300">{item}</span>
                      </div>
                    ))}
                  </div>
                </div>
                <motion.div className="absolute bottom-0 left-0 h-1 bg-emerald-500" initial={{ width: 0 }} whileHover={{ width: "100%" }} transition={{ duration: 0.5 }} />
              </motion.div>
            ))}
          </div>
        </div>
      ...
      {/* 04. PHOTOGRAPHY */}
      <section id="photography" className="relative z-30 bg-zinc-950 px-6 md:px-20 pt-12 pb-24 border-t border-zinc-900/50">
        <div className="max-w-7xl mx-auto flex flex-col items-center mb-14 text-center">
          <span className="text-[10px] uppercase tracking-[0.5em] font-bold text-zinc-500 opacity-40 mb-8 block">04 / Visuals</span>
          ...
          <Link to="/photography" className="mt-10 group flex items-center gap-4 px-8 py-4 rounded-full border border-zinc-800 hover:border-emerald-500 transition-all">
      ...
      {/* FOOTER */}
      <footer id="contact" className="relative z-30 bg-zinc-950 border-t border-zinc-900 pt-24 pb-10">
        <div className="max-w-7xl mx-auto px-6 md:px-20">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 mb-16">
            <div>
              ...
              <h2 className="text-5xl md:text-8xl font-black tracking-tighter text-white leading-none">
                LET&apos;S BUILD <br /> <span className="text-emerald-500">SOMETHING.</span>
              </h2>
